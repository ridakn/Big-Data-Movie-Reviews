...
if __name__ == '__main__':
    ...
    # Read in reviews and movies as CSV then convert to RDDs and repartition to 4 or 8 paritions 
    
    reviews = sess.read.csv('/Users/ridakhan/Desktop/reviews.csv', header=True).rdd
    reviews2 = reviews.repartition(4)
    #reviews2 = reviews.repartition(8)
    
    movies = sess.read.csv('/Users/ridakhan/Desktop/movies.csv', header=True).rdd
    movies2 = reviews.repartition(4)
    #movies2 = reviews.repartition(8)

    # Call task 1 with both files
    final = task1(reviews2, movies2)
